...
def intro():  # вывод вступительной речи
    print("Welcome to 'Hangman'")
    print("Rules: a New Year's theme word has been guessed, and you need to guess it "
          "letter by letter. You have 10 attempts and 2 hints.")
    print("Want to use hint? Write: hint")
    print("".join(goal_word))


def choose_hint():  # принятие выбранной подсказки
    while True:
...
def hint2(random_word, guessed_letters):
    ...
    if random.random() <= chance:
        ...
        if not_in_word:
            ...
            print(f"-Friend: Yo, man, im not really sure, but i think you should try letter'{incorrect_letter}'.")
    else:
        for letter in random_word:
            if letter not in guessed_letters:
                print(f"-Friend: Yo, man, im not really sure, but i think you should try letter '{letter}'.")
                return

def hint_use(random_word, guessed_letters):  # использование подсказки
    hint = choose_hint()
    if hint == "1":
...
def check(random_word, goal_word):  # проверка ответа
    global wrong_amount
